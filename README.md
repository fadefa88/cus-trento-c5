...
├── sitemap.xml
├── package.json
├── news/                  # pagine SEO generate
├── squadra/, staff/, calendario/, classifica/, statistiche/, coppa/
├── css/
│   └── style.css
├── js/
│   └── app.js
...
The app now supports clean SEO paths in addition to the old hash/query links, for example:
...
There is no server-side rendering and no database dependency. Static SEO entry pages are generated from the same JSON content before deploy, then the client-side app takes over when JavaScript loads.


## Static SEO generation

SEO pages, `sitemap.xml` and `robots.txt` are generated by:

```text
scripts/generate_static_pages.py
```

The script reads `content/data.json`, overlays the files under `content/cms/`, merges imported news from `content/news.index.json` and `content/news.imported.json`, and writes clean-path pages such as:
...
which calls `scripts/build_cloudflare_pages.sh`, regenerates static SEO pages and publishes the `_site` folder. The scheduled/news workflows also run the generator before committing, so newly imported news get a new `/news/<slug>/` path and appear in `sitemap.xml`.
