...
function setSquadTeam(team){view.squadTeam=team;view.squadPage=1;squad();}
function squad(){
  const roles=["Tutti","Portiere","Centrale","Laterale","Pivot","Universale"];
  const team=view.squadTeam||"Prima squadra";
  view.squadTeam=team;
  view.squadPage=view.squadPage||1;
  shell("Team",`Rosa — ${team}`,`<div class="team-switch squad-team-switch"><button class="${team==='Prima squadra'?'active':''}" onclick="setSquadTeam('Prima squadra')">Prima squadra</button><button class="${team==='Under 21'?'active':''}" onclick="setSquadTeam('Under 21')">Under 21</button></div><div class="toolbar">${roles.map(r=>`<button class="pill rolef ${r==="Tutti"?"active":""}">${r}</button>`).join("")}</div><div class="grid grid-4" id="squadGrid"></div><div id="squadPager"></div>`,"",`Rosa ${team} del CUS Trento C5 con profili giocatore e statistiche.`);
