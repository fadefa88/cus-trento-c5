(function(){
  ...
  function heroSection(){
    ...
    return `<section class="hero"><div class="container hero-grid"><div><span class="tag">Serie C1</span><span class="tag ghost">Stagione 26/27</span><h1>More than a team. <span class="red">Trento</span> plays fast.</h1><p class="lead"><br><br></p><p class="lead"></p><div class="btns"><button class="btn" onclick="route('fixtures')">Prossima partita →</button><button class="btn light" onclick="route('squad')">Scopri la rosa</button><button class="btn ghost" onclick="route('matchday')">Info matchday</button></div></div><div><div class="match-card"><div class="kicker">Prossima partita</div><div class="match-teams"><div><div class="clubmark">${h(leftMark || "CUS")}</div><b>${h(next.home || "CUS Trento")}</b></div><div><small>${h(next.date ? localFmt(next.date) : "Da definire")}</small><div class="timebox">${h(next.time || "--:--")}</div></div><div><div class="clubmark">${h(rightMark || "AV")}</div><b>${h(next.away || "Avversario")}</b></div></div></div><div class="mini-grid"><div class="mini"><strong>#${h(cus.pos || "-")}</strong><span>Posizione<br>${cus.pts != null ? h(cus.pts) + " pt" : ""}</span></div><div class="mini"><strong>${h(top.goals || 0)}</strong><span>Top scorer<br>${h(top.name || "")}</span></div><div class="mini"><strong>${h(last.score || "-")}</strong><span>Ultimo risultato</span></div></div></div></div></section>`;
  }
